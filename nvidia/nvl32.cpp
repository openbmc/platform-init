...
void power_on_platform()
{
    // assert to the host that BMC is ready
    gpio::set_raw(0, 8, 1);
    gpio::set_raw(1, 34, 1);

    // deal with pass through registers
    // and assert the power button for poweron
    handle_passthrough_registers(false);
    gpio::set_raw(0, 121, 0);
    std::this_thread::sleep_for(std::chrono::seconds{1});
    gpio::set_raw(0, 121, 1);
    handle_passthrough_registers(true);
}
...
int init_nvl32()
{
    power_on_platform();
    sd_notify(0, "READY=1");
    // change this to the more intelligent wait once it works
    //	dumb_wait();
