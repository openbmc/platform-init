...
constexpr std::array<unsigned, 2> hpm_iox_buses = {2, 7};
constexpr std::array<unsigned, 2> hpm_iox_addrs = {0x20, 0x21};
...
int init_vr_nvl()
{
    ...
    for (unsigned bus : hpm_iox_buses)
    {
        for (unsigned addr : hpm_iox_addrs)
        {
            bind_iox(bus, addr);
        }
    }
